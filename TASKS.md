...
1. **Boshlash uchun mock feature'lar (loyiha boshida ishlatilgan, endi olib tashlangan)** — dastlab `src/config.py`da `make_dummy_features(signals_df)` bo'lgan (tasodifiy shovqin bilan shartnomadagi ustunlarni generatsiya qiluvchi), Track A tugashini kutmasdan Track B ishlay boshlashi uchun. Ikkala trek ham tugagach, bu funksiya **audit orqali xavfli deb topilib olib tashlandi**: `train.py`/`predict.py` fayl topilmasa shu tasodifiy ma'lumotga sekin sirg'alib o'tar edi (faqat `print()` ogohlantirish bilan, xatosiz) — bu "haqiqiy ma'lumot o'rniga shovqinda o'qitilgan model.pkl birov bilmasdan submission sifatida yozilishi" xavfini tug'dirardi. Endi ikkalasi ham fayl topilmasa `FileNotFoundError` bilan qat'iy to'xtaydi.

2. **Model (`src/model.py`)** — yakuniy natija: gradient boosting emas, **kalibrlangan Logistic Regression** tanlandi (real CV'da 0.5629 vs HGB'ning 0.5396 AUC'i, keyinchalik tuned LightGBM ham 0.5513da qoldi — barchasi `scripts/model_selection_experiments.py` bilan qayta ishlab chiqariladi, batafsili README.md § 5.1/5.1.1). Xulosa: bu maydonda "murakkabroq model" degani "yaxshiroq" degani emas, doim bir nechta model turini real CV bilan solishtirib ko'ring — **va natijalarni hujjatlarga yozishdan oldin ishlatilgan kodni commit qiling**, aks holda ular tekshirib bo'lmaydigan da'voga aylanadi (bu loyihaning o'zida audit orqali topilgan xato).
